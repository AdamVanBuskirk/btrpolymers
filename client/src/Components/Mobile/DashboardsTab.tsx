...
function DashboardsTab() {
  ...
  // ✅ include if includeInReports is missing/undefined OR true; exclude only if explicitly false
  ...
  const filteredOutreach = useMemo(() => {
    ...
    return outreach.filter((o: any) => {
      ...
      // ✅ includeInReports gate (exclude only explicit false)
      ...
      if (dataTeams.length > 0) {
        const owner = memberByUserId[String(p.userId)] || null;
  ...
  const actionSummaryData = useMemo(() => {
    ...
    filteredOutreach.forEach((o: any) => {
      o.actions?.forEach((a: any) => {
        let key = a.name;
        const action = companyState.actions.find((act: any) => act._id === a.name);
        if (action) {
          key = action.name;
          const companyAction = companyState.companyActions.find((c: any) => c.actionId === action._id);
          if (companyAction) key = companyAction.name;
        }

        const val = String(a.value).toLowerCase().trim();
        if (!isNaN(Number(val)) && val !== "") counts[key] = (counts[key] || 0) + Number(val);
        else if (val === "true") counts[key] = (counts[key] || 0) + 1;
      });
    });
    ...
    filteredOutreach.forEach((o: any) => {
      o.actions?.forEach((a: any) => {
        ...
        if (!callType) {
          ...
          if (callTypeId) callType = companyState.actions.find((act: any) => act._id === callTypeId._id);
        }
  ...
  const opportunity = companyState.amounts.find((a: any) => a.label === "New Opportunity $ Value");
  if (opportunity) {
    opportunity_guid = opportunity._id;
    const companyOpportunity = companyState.companyAmounts.find((a: any) => a.actionId === opportunity?._id);
    if (companyOpportunity) opportunity_label = companyOpportunity.label;
  }

  const quotedAmt = companyState.amounts.find((a: any) => a.label === "Quote or Proposal Amount");
  if (quotedAmt) {
    quoted_guid = quotedAmt._id;
    const companyQuoted = companyState.companyAmounts.find((a: any) => a.actionId === quotedAmt?._id);
    if (companyQuoted) quoted_label = companyQuoted.label;
  }

  const closedAmt = companyState.amounts.find((a: any) => a.label === "Closed Amount");
  if (closedAmt) {
    closed_guid = closedAmt._id;
    const companyClosed = companyState.companyAmounts.find((a: any) => a.actionId === closedAmt?._id);
    if (companyClosed) closed_label = companyClosed.label;
  }

  const annualizedAmt = companyState.amounts.find((a: any) => a.label === "Annualized Value");
  if (annualizedAmt) {
    annualized_guid = annualizedAmt._id;
    const companyAnnualized = companyState.companyAmounts.find((a: any) => a.actionId === annualizedAmt?._id);
  ...
  const totals = useMemo(() => {
    ...
    filteredOutreach.forEach((o: any) => {
      o.amounts?.forEach((amt: any) => {
        ...
        if (amt.name === quoted_guid) quoted += val;
        if (amt.name === closed_guid) closed += val;
        if (amt.name === annualized_guid) annualized += val;
      });
    });

    return {
      newOpp: formatDollars(newOpp),
      quoted: formatDollars(quoted),
      closed: formatDollars(closed),
      annualized: formatDollars(annualized),
    };
  }, [filteredOutreach, opportunity_guid, quoted_guid, closed_guid, annualized_guid]);

  return (
    <div style={{ padding: "12px 12px 100px 12px" }}>
      <DataFilters
      ...
      {filteredOutreach.length === 0 ? (
        <div style={{ margin: "100px auto", textAlign: "center", fontStyle: "italic", color: "#555" }}>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
            {[
              { label: opportunity_label, value: totals.newOpp, color: "#2563eb" },
              { label: annualized_label, value: totals.annualized, color: "#0891b2" },
              { label: quoted_label, value: totals.quoted, color: "#f97316" },
              { label: closed_label, value: totals.closed, color: "#16a34a" },
            ].map((t) => (
              <div
                key={t.label}
                style={{
              ...
              >
                <h3 style={{ color: t.color, margin: "0", fontSize: "1rem" }}>{t.value}</h3>
                <p style={{ fontSize: "0.8rem", color: "#555" }}>{t.label}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "#fff", padding: "12px", borderRadius: "12px", marginBottom: "16px" }}>
            <h4 style={{ textAlign: "center", marginBottom: "6px" }}>Action Summary</h4>
          ...
          {/* Call Types */}
          <div style={{ background: "#fff", padding: "12px", borderRadius: "12px", marginBottom: "16px" }}>
            <h4 style={{ textAlign: "center", marginBottom: "6px" }}>Call Types</h4>
            <ResponsiveContainer width="100%" height={220}>
          ...
          {/* Client/Prospect Types */}
          <div style={{ background: "#fff", padding: "12px", borderRadius: "12px", marginBottom: "16px" }}>
            <h4 style={{ textAlign: "center", marginBottom: "6px" }}>Client / Prospect Types</h4>
            <ResponsiveContainer width="100%" height={220}>
          ...
          <div style={{ background: "#fff", padding: "12px", borderRadius: "12px" }}>
            <h4 style={{ textAlign: "center", marginBottom: "6px" }}>Monthly Actions & Revenue</h4>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={monthlyData} margin={{ top: 20, right: 10, left: 20, bottom: 5 }}>
                ...
                <Bar yAxisId="left" dataKey="newOpp" stackId="a" fill="#2563eb" name="New Opp" />
                <Bar yAxisId="left" dataKey="quoted" stackId="a" fill="#f97316" name="Quoted" />
                <Bar yAxisId="left" dataKey="closed" stackId="a" fill="#16a34a" name="Closed" />
